...
# Import billing and network agents
try:
    from billing_agent import create_billing_agent
    from network_agent import create_network_agent
    AGENTS_AVAILABLE = True
    print("✅ Billing and Network agents loaded")
except Exception as e:
...
ROOT_DIR = Path(__file__).resolve().parent
MODEL_PATH = ROOT_DIR / "district_knn_model.joblib"
CSV_DATA_PATH = ROOT_DIR / "slt_dummy_data.csv"
...
# -----------------------------
# Load CSV data for billing/network detection
# -----------------------------
PHONE_ISSUE_LOOKUP: Dict[str, Dict[str, str]] = {}

def load_phone_issue_data():
    """Load phone number issue data from CSV file."""
    global PHONE_ISSUE_LOOKUP
    if not CSV_DATA_PATH.exists():
        print(f"⚠️ CSV data file not found at {CSV_DATA_PATH}. Billing/Network detection will be disabled.")
        return
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if phone:
                ...
                billing_issue = str(row.get("Billing Issue", "")).strip()
                network_issue = str(row.get("Network Issue", "")).strip()
                
                PHONE_ISSUE_LOOKUP[phone_norm] = {
                    "billing_issue": billing_issue if billing_issue and billing_issue.lower() != "nan" else None,
                    "network_issue": network_issue if network_issue and network_issue.lower() != "nan" else None,
                    "location": str(row.get("Location", "")).strip()
                }
        print(f"✅ Loaded {len(PHONE_ISSUE_LOOKUP)} phone numbers from CSV")
    except Exception as e:
        print(f"❌ Error loading CSV data: {e}")
        import traceback
        traceback.print_exc()

load_phone_issue_data()

def detect_issue_type(phone_norm: str) -> Dict[str, Optional[str]]:
    """
    Detect if the phone number has a billing or network issue from CSV data.
    Returns dict with 'issue_type' ('billing', 'network', 'both', or None) and details.
    """
    phone_data = PHONE_ISSUE_LOOKUP.get(phone_norm, {})
    billing_issue = phone_data.get("billing_issue")
    network_issue = phone_data.get("network_issue")
    
    if billing_issue and network_issue:
        return {
            "issue_type": "both",
...
# -----------------------------
class Session:
    def __init__(self, district: str, weather: str, issue_info: Dict[str, Optional[str]] = None):
        self.district = district
        self.weather = weather
        self.issue_info = issue_info or {"issue_type": None, "billing_issue": None, "network_issue": None}
        self.issue_type = self.issue_info.get("issue_type")
        self.billing_issue = self.issue_info.get("billing_issue")
        self.network_issue = self.issue_info.get("network_issue")
        ...
        if OLLAMA_AVAILABLE:
            self.conversation, self.memory = create_chatbot(
                ...
                network_issue=self.network_issue
            )
        else:
            self.conversation = None
            self.memory = None
...
def init(req: InitRequest):
    try:
        ...
        # Detect billing/network issue from CSV data
        issue_info = detect_issue_type(phone_norm)
        if issue_info.get("issue_type"):
            print(f"🔍 Detected issue type: {issue_info.get('issue_type')} for phone {phone_norm}")
            if issue_info.get("billing_issue"):
                print(f"   Billing issue: {issue_info.get('billing_issue')}")
            if issue_info.get("network_issue"):
                print(f"   Network issue: {issue_info.get('network_issue')}")

        # Create chat session
        try:
            sid = str(uuid.uuid4())
            session = Session(district=district, weather=weather, issue_info=issue_info)
            SESSIONS[sid] = session
        except Exception as e:
            print(f"❌ Session creation error: {e}")
            raise HTTPException(status_code=500, detail=f"Session creation failed: {e}")
        ...
        # Generate first question using the chatbot
        try:
            # If billing/network issue detected, use specific prompt to ensure correct first question
            if issue_info.get("issue_type") == "billing" and issue_info.get("billing_issue"):
                first_prompt = f"Start by informing the customer about the billing issue: {issue_info.get('billing_issue')}. Then ask which service is affected by this billing issue."
            elif issue_info.get("issue_type") == "network" and issue_info.get("network_issue"):
                first_prompt = f"Start by informing the customer about the network issue: {issue_info.get('network_issue')}. Then ask which service is affected by this network issue."
            elif issue_info.get("issue_type") == "both":
                first_prompt = f"Start by informing the customer about both issues: billing ({issue_info.get('billing_issue')}) and network ({issue_info.get('network_issue')}). Then ask which they want to address first."
            else:
                first_prompt = f"Start asking questions to identify their telecom issue."
            ...
            print(f"❌ Error generating first question: {e}")
            # Fallback to static question based on detected issue
            if issue_info.get("issue_type") == "billing" and issue_info.get("billing_issue"):
                first_response = f"I see you have a billing issue: {issue_info.get('billing_issue')}. Which service is affected by this billing issue - WiFi, Internet, Landline, or Mobile?"
            elif issue_info.get("issue_type") == "network" and issue_info.get("network_issue"):
                first_response = f"I see there's a network issue: {issue_info.get('network_issue')}. Which service is affected by this network issue - WiFi, Internet, Landline, or Mobile?"
            elif issue_info.get("issue_type") == "both":
                first_response = f"I see you have both a billing issue ({issue_info.get('billing_issue')}) and a network issue ({issue_info.get('network_issue')}). Which would you like to address first - your billing issue or network issue?"
            else:
                # Fallback to weather-based question
                weather_lower = weather.lower()
                is_bad_weather = any(word in weather_lower for word in ["rain", "storm", "thunder", "heavy", "severe", "stormy", "windy"])
                if is_bad_weather:
                    first_response = (
        ...
        system_parts = [f"Detected district: {district}", f"Weather: {weather}"]
        if issue_info.get("issue_type"):
            if issue_info.get("issue_type") == "billing":
                system_parts.append(f"Billing issue detected: {issue_info.get('billing_issue')}")
            elif issue_info.get("issue_type") == "network":
                system_parts.append(f"Network issue detected: {issue_info.get('network_issue')}")
            elif issue_info.get("issue_type") == "both":
                system_parts.append(f"Both billing and network issues detected")
        system_parts.append("I will ask up to 5 questions to identify your issue.")
...
def chat(req: ChatRequest):
    ...
    # If we're at turn 4, remind the model to return JSON next turn (5th question) with the service type
    if session.turns == 4:
        user_message = (
            f"{user_message} [REMINDER: This will be your 5th and final question. On your next response, you MUST return JSON "
            f"with the correct sub_category based on the service type the user mentioned earlier "
            f"(WiFi/Internet/Landline/Mobile). Do NOT return Unknown.]"
        )
    ...
    try:
        ...
        # Enforce max 5 questions (after 5 questions, force JSON response)
        if session.turns >= 5:
            try:
                final_prompt = (
                    'You must now return JSON with issue_type and sub_category. '
